      ...
      let
        ...
        mergeContainerQuadlet =
          ...
          let
            ...
            # System-rendered secrets (e.g. sops-nix templates) can't be expressed as a systemd
            # `After=`/`Requires=` dependency here, since this unit is generated by Podman's
            # Quadlet generator inside a per-user systemd instance, which can't order against units
            # in the system instance. Instead, block container start until every referenced
            # EnvironmentFile actually exists, so a slow/racing secrets render doesn't just fail
            # the container outright.
            environmentFiles = lib.toList (existingContainer.EnvironmentFile or [ ]);

            waitForEnvironmentFilesScript = pkgs.writeShellScript "${deploymentPrefix}-${containerName}-wait-for-secrets" ''
              for f in ${lib.concatMapStringsSep " " lib.escapeShellArg environmentFiles}; do
                until [ -e "$f" ]; do
                  echo "Waiting for $f to exist..."
                  sleep 1
                done
              done
            '';
          ...
          // {
            ...
            Service =
              ...
              // lib.optionalAttrs (environmentFiles != [ ]) {
                ExecStartPre = lib.toList (existingService.ExecStartPre or [ ]) ++ [
                  "${waitForEnvironmentFilesScript}"
                ];
              };
          };
